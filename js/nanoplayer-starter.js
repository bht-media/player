...
function startPlayerSimpleConfig(playerDiv, stream) {
  ...
  if (typeof (stream) === "string") {
    ...
    //set "titleTop" element text
    let titleTop = document.getElementById('titleTop');
    if (titleTop && stream.titleTop) {
      console.log("set title : " + stream.titleTop);
      titleTop.innerText = stream.titleTop;
    } else {
      console.log("set title standard ");
      titleTop.innerText = "Beuth Hochschule für Technik - Stream";
    }

    // set "titleSub" element text
    let titleSub = document.getElementById('titleSub');
    if (titleSub && stream.title) titleSub.innerText = stream.title;
    // set Brückenkurs-Button visibility
    console.log("INDEX: " + stream.tag + " | " + stream.tag.indexOf("ckenkurs"));
